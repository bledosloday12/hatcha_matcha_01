...
def main() -> None:
    ...
    L('          <textarea id="entropy" rows="3" placeholder="leave blank for crypto.getRandomValues"></textarea>')
    L("        </div>")
    L('        <div class="card">')
    L("          <h2>League telemetry</h2>")
    L('          <div id="stats" class="log"></div>')
    L('          <div class="row" style="margin-top:10px;">')
    L('            <button class="ghost" id="btnExport" type="button">Export JSON</button>')
    L('            <button class="ghost" id="btnImport" type="button">Import JSON</button>')
    L('            <button class="ghost" id="btnCsv" type="button">Export CSV</button>')
    L("          </div>")
    L('          <textarea id="io" rows="4" placeholder="paste roster JSON here"></textarea>')
    L("        </div>")
    L("      </section>")
    L('      <section class="card">')
    L("        <h2>Battle log</h2>")
    L('        <div id="log" class="log"></div>')
    L("      </section>")
    L('      <section class="card">')
    L("        <h2>Species almanac</h2>")
    L('        <div style="max-height:360px; overflow:auto;">')
    L("          <table>")
    L("            <thead><tr><th>#</th><th>Name</th><th>Might</th><th>Guard</th><th>Tempo</th><th>Elem</th></tr></thead>")
    L("            <tbody>")
    for i, (n, a, d, s, e) in enumerate(SPECIES, start=1):
        L(f'              <tr><td>{i}</td><td>{n}</td><td>{a}</td><td>{d}</td><td>{s}</td><td class="elem{e}">{e}</td></tr>')
    L("            </tbody>")
    L("          </table>")
    L("        </div>")
    L("      </section>")
    L('      <section class="card">')
    L("        <h2>Move catalog</h2>")
    L('        <p class="micro">Slot codes 1–32 mirror the chikn_LIKN lexicon for coaching drills.</p>')
    L('        <div style="max-height:320px; overflow:auto;">')
    L("          <table>")
    L("            <thead><tr><th>Code</th><th>Label</th></tr></thead>")
    L("            <tbody>")
    for code, label in MOVE_NAMES:
        L(f"              <tr><td>{code}</td><td>{label}</td></tr>")
    L("            </tbody>")
    L("          </table>")
    L("        </div>")
    L("      </section>")
    L('      <section class="card">')
    L("        <h2>Advantage lattice (attacker rows vs defender columns)</h2>")
    L('        <p class="micro">+1 advantage, -1 disadvantage, 0 neutral — training wheels for elemental rings.</p>')
    L('        <div style="max-height:380px; overflow:auto;">')
    L("          <table>")
    L('            <thead><tr><th>Atk\\Def</th>')
    for h in range(1, 9):
        L(f"              <th>{h}</th>")
    L("            </tr></thead>")
    L("            <tbody>")
    for atk in range(1, 9):
        L(f"              <tr><th>{atk}</th>")
        for de in range(1, 9):
            v = type_advantage_py(atk, de)
            cls = "pos" if v > 0 else "neg" if v < 0 else "zero"
            L(f'                <td class="{cls}">{v}</td>')
        L("              </tr>")
    L("            </tbody>")
    L("          </table>")
    L("        </div>")
    L("      </section>")
    L('      <section class="card">')
    L("        <h2>Coaching playbook</h2>")
    L('        <p class="hero">Micro-lessons from the league road crew — read between spars.</p>')
    L('        <div class="cols">')
    half = (len(COACHING_TIPS) + 1) // 2
    for chunk in (COACHING_TIPS[:half], COACHING_TIPS[half:]):
        L('          <div>')
        for tip in chunk:
            L(f'            <p class="tip">{tip}</p>')
        L("          </div>")
    L("        </div>")
    L("      </section>")
    L('      <section class="card">')
    L("        <h2>Drill reference</h2>")
    L('        <p class="tip">Feed cadence <span class="kbd">120s</span> · Train cadence <span class="kbd">240s</span> · Spar cadence <span class="kbd">90s</span> on-chain.</p>')
    L('        <div class="split"></div>')
    L('        <p class="tip">Python trainer: run <span class="kbd">python chikn_LIKN.py</span> for REPL commands (<span class="kbd">HELP</span> lists verbs).</p>')
    L('        <p class="tip">Self-test hook: <span class="kbd">set CHIKN_SELFTEST=1</span> then execute module.</p>')
    L('        <p class="tip">Contract: <span class="kbd">ChickCombo.sol</span> — review NatSpec before mainnet deploy.</p>')
    L('        <p class="tip">Telemetry: league banner lines echo shortened hints for ADDRESS immutables.</p>')
    L('        <p class="tip">Safety: disable browser extensions that auto-inject scripts on local files.</p>')
    L('        <p class="tip">Storage: keep JSON backups under version control with private repos only.</p>')
    L('        <p class="tip">QA: after UI edits, reload hard-refresh to bust cached scripts.</p>')
    L('        <p class="tip">Perf: close devtools overlays when recording demo GIFs.</p>')
    L("      </section>")
    L('      <section class="card">')
    L("        <h2>Arena rules & integrator appendix</h2>")
    L('        <p class="micro">Operational gates for humans shipping ChickCombo adjacent tools.</p>')
    L("        <ol>")
    for rule in ARENA_RULES:
        L(f"          <li class=\"tip\">{rule}</li>")
    L("        </ol>")
    L("      </section>")
    L('      <section class="card">')
    L("        <h2>Doubles synergy sketches</h2>")
    L('        <p class="micro">Narrative pairings for scrimmage storylines — not enforced on-chain.</p>')
    L("        <ul>")
    for duo in DOUBLES_SYNERGY:
        L(f'          <li class="tip">{duo}</li>')
